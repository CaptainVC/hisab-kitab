...
export async function main() {
    ...
    const cache = readJson(cacheFp, null);
    if (!cache)
        throw new Error(`missing_cache:${cacheFp}`);
    ...
    for (const mo of store.orders) {
        ...
        for (const [d, list] of byDate.entries()) {
            ...
            if (dayDelta === null)
                continue;
            if (Math.abs(dayDelta) > bufferDays)
                continue;
            for (const r of list) {
        ...
        if (candidates.length === 0) {
            none++;
            if (noneExamples.length < 10)
                noneExamples.push({ mail: mo });
            continue;
        }
...
//# sourceMappingURL=mail_crossref.js.map