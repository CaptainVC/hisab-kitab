...
export async function registerMailRoutes(app: FastifyInstance, opts: { baseDir: string; repoDir: string; stagingDir: string; runner: JobRunner }) {
  ...
  app.get('/api/v1/mail/merchants', async (req, reply) => {
    ...
    const by: Record<string, { count: number; oldest: string | null; newest: string | null; withItems: number }> = {};
    for (const o of orders) {
      const mc = String(o?.merchant_code || '').trim().toUpperCase() || 'UNKNOWN';
      const d = String(o?.date || '').slice(0, 10);
      if (!by[mc]) by[mc] = { count: 0, oldest: null, newest: null, withItems: 0 };
      by[mc].count++;
      if (d) {
        if (!by[mc].oldest || d < by[mc].oldest) by[mc].oldest = d;
        if (!by[mc].newest || d > by[mc].newest) by[mc].newest = d;
      }
      const items = Array.isArray(o?.items) ? o.items : [];
      if (items.length) by[mc].withItems++;
    }
  ...
  app.get('/api/v1/mail/reconcile/report/:reportId', async (req, reply) => {
    ...
    if (!id) return reply.code(400).send({ ok: false, error: 'missing_report_id' });
  ...
  app.get('/api/v1/mail/crossrefOrders', async (req, reply) => {
    ...
    if (!r) return reply.code(400).send({ ok: false, error: 'bad_range' });
