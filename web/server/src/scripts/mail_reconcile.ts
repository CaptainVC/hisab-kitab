...
function readJson(fp: string, fallback: any) {
...
function hasTag(tags: any, t: string) {
...
type MailOrder = { messageId: string; date: string; total: number; items: Array<{ name: string; amount: number }>; merchant_code: string };
...
function loadMailOrders(baseDir: string, merchantCode: string, from: string, to: string) {
  const fp = path.join(baseDir, 'staging', 'mail_orders.json');
  const store = readJson(fp, { schemaVersion: 1, orders: [] });
  const orders = Array.isArray(store.orders) ? store.orders : [];

  return orders
    .filter((o: any) => String(o?.merchant_code || '').toUpperCase() === merchantCode)
    .map((o: any) => {
      ...
      return {
        ...
        items: items
          .map((it: any) => ({ name: String(it.name || it.title || '').trim(), amount: Number(it.amount || it.total || 0) }))
          .filter((it: any) => it.name && Number.isFinite(it.amount) && it.amount > 0),
        merchant_code: merchantCode
      } as MailOrder;
    })
    .filter((o: any) => o.date && Number.isFinite(o.total) && o.total > 0)
    .filter((o: any) => inRange(o.date, from, to));
}

function loadTxnCandidates(baseDir: string, merchantCode: string, from: string, to: string, includeRawMention: boolean) {
  const XLSX = require('xlsx');
  const files = listQuarterlyFiles(baseDir);
  const reMention = new RegExp(`\\b${merchantCode.toLowerCase()}\\b`, 'i');

  const out: TxnCandidate[] = [];
  ...
  for (const fp of files) {
    ...
    for (const sheet of wb.SheetNames) {
      ...
      for (const r of rows) {
        ...
        const rowCat = String((r as any).category || '');
        const rowCatName = String((r as any).category_name || '');
        const normType = (rowCat === 'TRANSFER' || rowCatName === 'Transfers') ? 'TRANSFER' : String((r as any).type || '');
        if (String(normType).toUpperCase() !== 'EXPENSE') continue;

        const amt = Number((r as any).amount || 0);
        if (!Number.isFinite(amt) || amt === 0) continue;

        const mc = String((r as any).merchant_code || '').trim().toUpperCase();
...
function reconcile(mailOrders: MailOrder[], txns: TxnCandidate[], bufferDays: number, tol: number) {
  const byDate = new Map<string, TxnCandidate[]>();
  for (const t of txns) {
    if (!byDate.has(t.date)) byDate.set(t.date, []);
    byDate.get(t.date)!.push(t);
  }

  function datesWithin(dateIso: string) {
    const out: Array<{ date: string; dd: number }> = [];
    for (const d of byDate.keys()) {
      const dd = daysBetween(dateIso, d);
      if (dd === null) continue;
      if (Math.abs(dd) <= bufferDays) out.push({ date: d, dd });
    }
