...
export default function IngestPage() {
  ...
  return (
    <div>
      ...
      <p className="text-[color:var(--hk-muted)] mt-1">Runs: poll_ingest + rebuild cache for the selected range.</p>
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 hk-card">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input className="mt-1 w-full px-2 py-1 rounded bg-zinc-900 border [var(--hk-border)]" value={from} onChange={e => { const v = e.target.value; setFrom(v); saveRange({ from: v, to }); }} />
            </div>
            <div>
              <label className="text-xs text-[color:var(--hk-muted)]">To (YYYY-MM)</label>
              <input className="mt-1 w-full px-2 py-1 rounded bg-zinc-900 border [var(--hk-border)]" value={to} onChange={e => { const v = e.target.value; setTo(v); saveRange({ from, to: v }); }} />
            </div>
            <div>
              <label className="text-xs text-[color:var(--hk-muted)]">Min confidence</label>
              <input className="mt-1 w-full px-2 py-1 rounded bg-zinc-900 border [var(--hk-border)]" type="number" step="0.01" value={minConfidence} onChange={e => setMinConfidence(Number(e.target.value))} />
            </div>
            <div>
              <label className="text-xs text-[color:var(--hk-muted)]">Max orders</label>
              <input className="mt-1 w-full px-2 py-1 rounded bg-zinc-900 border [var(--hk-border)]" type="number" value={maxOrders} onChange={e => setMaxOrders(Number(e.target.value))} />
            </div>
            <div>
              <label className="text-xs text-[color:var(--hk-muted)]">Max payments</label>
              <input className="mt-1 w-full px-2 py-1 rounded bg-zinc-900 border [var(--hk-border)]" type="number" value={maxPayments} onChange={e => setMaxPayments(Number(e.target.value))} />
            </div>
            <div className="flex items-end gap-2">
              <label className="text-sm text-[color:var(--hk-muted)] flex items-center gap-2">
          ...
          <button
            className="mt-4 px-3 py-2 hk-btn-primary disabled:opacity-50"
            disabled={busy}
            onClick={startIngest}
          >
        ...
        <div className="p-4 hk-card">
          <div className="flex items-center justify-between">
            ...
            <button className="text-sm text-[color:var(--hk-muted)] hover:text-white" onClick={() => refreshJobs().catch(() => {})}>Refresh</button>
          </div>
          <div className="mt-2 space-y-2 max-h-64 overflow-auto">
            {jobs.map(j => (
              <button
                key={j.jobId}
                className={`w-full text-left px-3 py-2 rounded border ${selectedJobId === j.jobId ? 'border-zinc-600 bg-zinc-900' : '[var(--hk-border)] hover:bg-zinc-900'}`}
                onClick={() => { setSelectedJobId(j.jobId); setLogText(''); setLogOffset(0); }}
              >
                <div className="flex items-center justify-between">
                  <div className="text-sm">{j.type} • {j.status}</div>
                  <div className="text-xs text-[color:var(--hk-faint)]">{new Date(j.createdAt).toLocaleString()}</div>
                </div>
                <div className="text-xs text-[color:var(--hk-faint)] mt-1">{j.jobId}</div>
              </button>
            ))}
            {jobs.length === 0 ? <div className="text-sm text-[color:var(--hk-faint)]">No jobs yet.</div> : null}
          </div>
        </div>
      </div>
      ...
      <div className="mt-6">
        ...
        <pre className="mt-2 p-3 hk-card bg-zinc-950 text-xs overflow-auto max-h-[420px] whitespace-pre-wrap">{logText || '(select a job to view logs)'}</pre>
      </div>
    </div>
  );
}
