#!/usr/bin/env node
/*
Add tag `for_others` to any transaction rows that have subcategory "Paid for someone else"
(typically subcategory code TRANSFER_FOR_OTHERS).

Edits HK_*.xlsx in-place under baseDir. Makes backups.

Usage:
  node sync_for_others_tag.js --base-dir /home/molt/HisabKitab [--dry-run]
*/
...
function main(){
  ...
  const stamp=ts();
  const backupDir=path.join(baseDir,'backups',`sync_for_others_${stamp}`);
  if(!dryRun) backupFiles(backupDir, hkFiles);

  const targetSubcats = new Set(['TRANSFER_FOR_OTHERS', 'PAID_FOR_SOMEONE_ELSE']);
  ...
  for(const fp of hkFiles){
    ...
    for(const sheetName of wb.SheetNames){
      ...
      // Column names differ sometimes. Prefer canonical.
      ...
      for(const r of rows){
        ...
        if(!sub) continue;
        if(!targetSubcats.has(sub)) continue;

        const tags=splitTags(r[kTags]);
        if(tags.includes('for_others')) continue;
        tags.push('for_others');
        r[kTags]=tags.join(',');
        rowsTagged++;
        sheetChanged=true;
      }
