...
async function main() {
  ...
  if (!patchFile) throw new Error('missing_patch_file');

  const patch = JSON.parse(fs.readFileSync(patchFile, 'utf8')) as any;

  const XLSX = require('xlsx');

  const files = listQuarterlyFiles(baseDir);
  let updated = 0;
  let foundIn: any = null;
  ...
  for (const fp of files) {
    ...
    for (const sheetName of wb.SheetNames) {
      ...
      const sample = rows[0] || {};
      const key = (a: string, b: string) => (Object.prototype.hasOwnProperty.call(sample, a) ? a : b);
