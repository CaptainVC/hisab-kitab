...
function writeStatus(refsDir: string, status: any) {
...
export async function main() {
  ...
  if (!refsDir) throw new Error('missing_refs_dir');
  ...
  if (last.ok) status.lastCommit = last.stdout.trim();

  status.ok = status.steps.every((s: any) => s.ok);
