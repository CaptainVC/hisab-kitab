...
function parseStatementTextToRows(txt) {
...
export async function registerStatementRoutes(app, opts) {
        ...
        try {
            ...
            const matches = stmtRows.map((s, idx) => {
                const candidates = hkRows
                    ...
                    if (!d)
                        return false;
                    if (Math.abs(Number(s.amount) - amt) > 0.01)
                        return false;
                    if (dayDiff(d, s.date) > daysWindow)
                        return false;
                    return true;
                })
                    .slice(0, 10)
                    .map((r) => ({ txn_id: r.txn_id, date: r.date, amount: r.amount, raw_text: r.raw_text || '', type: r.type || '' }));
                return {
                    idx,
                    date: s.date,
                    amount: s.amount,
