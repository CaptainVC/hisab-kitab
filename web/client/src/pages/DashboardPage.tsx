      ...
      {editTxnOpen ? (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50" onClick={() => setEditTxnOpen(false)}>
          <div className="w-full max-w-lg hk-card p-4" onClick={(e) => e.stopPropagation()}>
            ...
            {splitOpen ? (
              <div className="mt-4 border-t [var(--hk-border)] pt-4">
                <div className="text-sm font-semibold">Split transaction</div>
                <div className="mt-1 text-xs text-[color:var(--hk-muted)]">
                  This will tag the original as <span className="font-mono">superseded</span> (excluded from dashboard totals) and append child transactions.
                </div>
                  ...
                  {splitLines.map((ln, idx) => (
                    <div key={idx} className="grid grid-cols-1 md:grid-cols-6 gap-2 items-end">
                      <div>
                        ...
                        <input className="mt-1 w-full hk-input" value={ln.amount} onChange={(e) => {
                          const v = e.target.value;
                          setSplitLines(xs => xs.map((x,i)=> i===idx?{...x, amount:v}:x));
                        }} placeholder="100" />
                      </div>
                      <div className="md:col-span-2">
                        <label className="text-xs text-[color:var(--hk-muted)]">Text</label>
                        <input className="mt-1 w-full hk-input" value={ln.raw_text} onChange={(e) => {
                          const v = e.target.value;
                          setSplitLines(xs => xs.map((x,i)=> i===idx?{...x, raw_text:v}:x));
                        }} placeholder="Item name" />
                      </div>
                      <div>
                        <label className="text-xs text-[color:var(--hk-muted)]">Merchant</label>
                        <input className="mt-1 w-full hk-input" value={ln.merchant_code} onChange={(e) => {
                          const v=e.target.value;
                          setSplitLines(xs => xs.map((x,i)=> i===idx?{...x, merchant_code:v}:x));
                        }} placeholder="SWIGGY" />
                      </div>
                      <div>
                        <label className="text-xs text-[color:var(--hk-muted)]">Category</label>
                        <input className="mt-1 w-full hk-input" value={ln.category} onChange={(e) => {
                          const v=e.target.value;
                          setSplitLines(xs => xs.map((x,i)=> i===idx?{...x, category:v}:x));
                        }} placeholder="FOOD_DINING" />
                      </div>
                      <div>
                        <label className="text-xs text-[color:var(--hk-muted)]">Subcat</label>
                        <input className="mt-1 w-full hk-input" value={ln.subcategory} onChange={(e) => {
                          const v=e.target.value;
                          setSplitLines(xs => xs.map((x,i)=> i===idx?{...x, subcategory:v}:x));
                        }} placeholder="FOOD_ONLINE_DELIVERY" />
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="mt-3 flex justify-between gap-2">
                  <button className="hk-btn-secondary" onClick={() => setSplitLines(xs => xs.concat([{ amount:'', raw_text:'', merchant_code: editMerchant, category: editCategory, subcategory: editSubcategory }]))}>
                    + Add line
                  </button>
                  <button
                    className="hk-btn-primary"
